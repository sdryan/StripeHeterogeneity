...
saveas(fig4,'P1N1_Initialization_Peaks.pdf')
...
% Find time corresponding to 25% phase shift
...
if sync_test == true
    ...
    % Prepare video
    fig5 = figure(5);
    vidfile = VideoWriter('synchronization_test_p1n1.mp4','MPEG-4');
    open(vidfile);
    ...
    for k = 1:Time/dt
        ...
        if (mod(k,500)==0)
            ...
            xlabel('Position')
            ylabel('Signal Amount')
            axis([0 L 0 0.1*10*basal_multiplier])
            ax = gca;
            ax.FontSize = 16; 
    ...
    title('P1N1 Signal Production vs. Time')
    xlabel('Time')
    ylabel('Signal Amount')
    axis([0 Time/dt 0 1])
    ax = gca;
    ...
    saveas(fig6,'P1N1_Concentration_Peaks.pdf')
    ...
    fprintf('The basal multiplier P1N1 is: %.2f\n',basal_multiplier);
    fprintf('The percent phase difference for P1N1 is: %.2f\n',phase_difference_percent);
